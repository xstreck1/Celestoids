...
public class SuperInputMapper
{
	...
	#region Mapping Helpers
		...
		public static float GreaterValue (float valIn, float compareVal)
		{
				...
				if (compareVal > 0f) {
						if (valIn < 0f && valIn * -1f < compareVal)
								return compareVal;
						if (valIn >= 0f && valIn < compareVal)
								return compareVal;
				} else {
						if (valIn > 0f && valIn * -1f > compareVal)
								return compareVal;
		...
		public static float GetAxis (OuyaSDK.KeyEnum keyCode, OuyaSDK.OuyaPlayer player)
		{
			...
			default:
				...
				default: //the driver is reporting the controller as blank
		...
		public static bool GetButton (OuyaSDK.KeyEnum keyCode, OuyaSDK.OuyaPlayer player)
		{
				...
				if (buttonIndex.buttonIndex == -10)
						return Input.GetKey ((KeyCode)319); //OUYA home button
		...
		public static bool GetButtonDown (OuyaSDK.KeyEnum keyCode, OuyaSDK.OuyaPlayer player)
		{
				...
				if (buttonIndex.buttonIndex == -1)
						return false; //no such button
				if (buttonIndex.buttonIndex == -10)
						return Input.GetKeyDown ((KeyCode)319); //OUYA home button
